...
export default function App() {
  ...
  const layer4AudioInputRef = useRef<HTMLInputElement>(null);
  ...
  const layer4AudioRef = useRef<HTMLAudioElement>(null);
  ...
  useEffect(() => {
    const loadAssets = async () => {
      ...
      const savedKtp = await getAsset('sosis_love_ktp');
      const savedLayer4Audio = await getAsset('sosis_love_layer4_audio');
  ...
  useEffect(() => {
    ...
    let layer4MusicInterval: NodeJS.Timeout | undefined;
    ...
    const isMenuOrSettings = state.status === 'menu' || state.status === 'settings' || state.status === 'about';
    const isGameActive = state.status === 'opening' || state.status === 'playing' || state.status === 'layer3';
    const isLayer4 = state.status === 'layer4';

    if (isMenuOrSettings) {
      // Fade in Menu Music
      if (state.soundtrack && soundtrackAudioRef.current) {
        if (soundtrackAudioRef.current.paused) {
          menuMusicInterval = fadeIn(soundtrackAudioRef.current, 2000, state.musicVolume);
        } else {
      ...
      if (gameSoundtrackAudioRef.current && !gameSoundtrackAudioRef.current.paused) {
        gameMusicFadeOut = fadeOut(gameSoundtrackAudioRef.current, 1000);
      }
      // Fade out Layer 4 Audio
      if (layer4AudioRef.current && !layer4AudioRef.current.paused) {
        fadeOut(layer4AudioRef.current, 1000);
      }
    } else if (isGameActive) {
      // Fade in Game Music
      if (state.gameSoundtrack && gameSoundtrackAudioRef.current) {
        if (gameSoundtrackAudioRef.current.paused) {
          gameMusicInterval = fadeIn(gameSoundtrackAudioRef.current, 1000, state.musicVolume);
        } else {
      ...
      if (soundtrackAudioRef.current && !soundtrackAudioRef.current.paused) {
        menuMusicFadeOut = fadeOut(soundtrackAudioRef.current, 500);
      }
      // Fade out Layer 4 Audio
      if (layer4AudioRef.current && !layer4AudioRef.current.paused) {
        fadeOut(layer4AudioRef.current, 1000);
      ...
      // Fade in Layer 4 Audio
      if (state.layer4Audio && layer4AudioRef.current) {
        if (layer4AudioRef.current.paused) {
          layer4MusicInterval = fadeIn(layer4AudioRef.current, 1000, state.musicVolume);
        } else {
          layer4AudioRef.current.volume = state.musicVolume;
        }
      }
      // Fade out Game Music
      if (gameSoundtrackAudioRef.current && !gameSoundtrackAudioRef.current.paused) {
    ...
    return () => {
      ...
      if (layer4MusicInterval) clearInterval(layer4MusicInterval);
      if (menuMusicFadeOut) clearInterval(menuMusicFadeOut);
      if (gameMusicFadeOut) clearInterval(gameMusicFadeOut);
    };
  }, [state.status, state.rainSound, state.soundtrack, state.gameSoundtrack, state.layer4Audio, userInteracted, state.musicVolume, state.rainVolume]);

  useEffect(() => {
    if (state.status === 'opening') {
      setOpeningStep(1);
      const t1 = setTimeout(() => setOpeningStep(2), 2500); // Fade out 1
      const t2 = setTimeout(() => setOpeningStep(3), 3500); // Fade in 2
      const t3 = setTimeout(() => {
        setState(prev => ({ ...prev, status: 'layer3' }));
      }, 7000); // Auto transition directly to Layer 3 (Image Feed)
      return () => {
        clearTimeout(t1);
        clearTimeout(t2);
        clearTimeout(t3);
      };
    } else if (state.status === 'layer3') {
      setOpeningStep(0);
      const timer = setTimeout(() => {
        setState(prev => ({ ...prev, status: 'layer4' }));
      }, 5000); // Show KTP for 5 seconds then transition
  ...
  // Auto-transition from Playing (Layer 2) to Layer 3 is no longer needed
  // as we go directly from opening to layer3.
  // Keeping the state logic for potential future use or manual navigation.
  
  const handleLayer4AudioUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (file) {
      if (file.size > 10 * 1024 * 1024) {
        alert("Audio terlalu besar! Maksimal 10MB.");
      ...
      reader.onloadend = async () => {
        const base64String = reader.result as string;
        setState(prev => ({ ...prev, layer4Audio: base64String }));
        try {
          await setAsset('sosis_love_layer4_audio', base64String);
          setUploadSuccess('bg'); 
        } catch (err) {
          console.error("Gagal menyimpan layer 4 audio:", err);
          alert("Gagal menyimpan layer 4 audio.");
        }
      };
      reader.readAsDataURL(file);
    }
  ...
  const handleKtpUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onloadend = async () => {
        const base64String = reader.result as string;
        setState(prev => ({ ...prev, ktpImage: base64String }));
        try {
          await setAsset('sosis_love_ktp', base64String);
          setUploadSuccess('bg');
        } catch (err) {
          console.error("Gagal menyimpan KTP:", err);
          alert("Gagal menyimpan KTP.");
        }
      };
      reader.readAsDataURL(file);
    }
  ...
  return (
    <>
      ...
      {/* Background with Overlay - Hide during game states */}
      {!(state.status === 'layer3' || state.status === 'layer4' || state.status === 'opening' || state.status === 'playing') && (
      ...
      {/* Main UI Container */}
      <div className={`relative z-10 h-full flex flex-col items-center justify-between ${state.status === 'layer3' || state.status === 'layer4' || state.status === 'opening' || state.status === 'playing' ? 'p-0' : 'pt-[6vh] pb-16 px-8'}`}>
        <AnimatePresence>
          {state.status === 'menu' ? (
            <motion.div 
              key="menu"
            ...
            <div className="w-full max-w-[280px] space-y-3">
              <motion.button
                onClick={() => {
                  setState(prev => ({ ...prev, status: 'opening' }));
                }}
              ...
              {state.isDevMode && (
                <div className="w-full">
                  <motion.button
                    ...
                    whileTap={{ scale: 0.98 }}
                    transition={{ type: "spring", stiffness: 400, damping: 17 }}
                    className="w-full py-3 bg-orange-600/20 border border-orange-500/30 rounded-full text-[10px] tracking-widest uppercase text-orange-400 hover:bg-orange-500/30 transition-colors flex items-center justify-center gap-2"
                  >
          ...
          <motion.div 
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="absolute inset-0 z-[80] bg-black flex flex-col items-center justify-center p-12 text-center"
            style={{ fontFamily: 'Arial, sans-serif' }}
          >
            <div className="max-w-2xl h-32 flex items-center justify-center relative">
              <AnimatePresence mode="wait">
                {openingStep === 1 && (
                  <motion.p
                    ...
                    className="text-xl md:text-3xl text-white/90 leading-relaxed font-medium tracking-wide italic"
                  >
                    "Katanya, waktu adalah tabib yang menyembuhkan segala luka."
                  </motion.p>
                )}
                ...
                {openingStep === 3 && (
                  <motion.p
                    ...
                    className="text-xl md:text-3xl text-white/90 leading-relaxed font-medium tracking-wide italic"
                  >
                    "Tapi bagiku, waktu adalah algojo yang sedang mengasah pisaunya..."
                  </motion.p>
                )}
              </AnimatePresence>
            </div>
          ...
          <motion.div 
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="absolute inset-0 z-[85] bg-black flex flex-col items-center justify-center text-center overflow-hidden"
          >
            {state.isDevMode && (
              <motion.div 
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                className="absolute top-8 left-8 right-8 bg-black/80 border border-orange-500/30 p-6 rounded-2xl backdrop-blur-xl z-[100]"
              >
                <div className="flex items-center justify-between mb-6 border-b border-white/10 pb-3">
                  <div className="flex items-center gap-3">
                    <Sliders className="text-orange-500" size={20} />
                    <h3 className="text-xs font-bold uppercase tracking-[0.3em] text-orange-400">Layer 2 Developer Mode</h3>
                  </div>
                  <motion.button
                  ...
                  >
                    ...
                    <span className="text-[9px] font-bold uppercase tracking-widest text-white/60 group-hover:text-white">Kembali ke Menu</span>
                  </motion.button>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                  <motion.button
                    ...
                    whileHover={{ scale: 1.02 }}
                    whileTap={{ scale: 0.98 }}
                    className="flex flex-col items-center justify-center gap-3 p-4 bg-white/5 border border-white/10 rounded-xl hover:bg-orange-500/10 hover:border-orange-500/30 transition-all group"
                  >
                    <Camera className="text-white/40 group-hover:text-orange-400 transition-colors" size={24} />
                    <span className="text-[10px] uppercase tracking-widest font-bold">Upload Background</span>
                  </motion.button>
                  ...
                  <motion.button
                    ...
                    className="flex flex-col items-center justify-center gap-3 p-4 bg-white/5 border border-white/10 rounded-xl hover:bg-orange-500/10 hover:border-orange-500/30 transition-all group"
                  >
                    <Play className="text-white/40 group-hover:text-orange-400 transition-colors" size={24} />
                    <span className="text-[10px] uppercase tracking-widest font-bold">Upload Video Overlay</span>
                  </motion.button>

                  <motion.button
                    onClick={() => gameSoundtrackInputRef.current?.click()}
                    whileHover={{ scale: 1.02 }}
                    whileTap={{ scale: 0.98 }}
                    className="flex flex-col items-center justify-center gap-3 p-4 bg-white/5 border border-white/10 rounded-xl hover:bg-orange-500/10 hover:border-orange-500/30 transition-all group"
                  >
                    <Music className="text-white/40 group-hover:text-orange-400 transition-colors" size={24} />
                    <span className="text-[10px] uppercase tracking-widest font-bold">Upload Game Music</span>
                  </motion.button>

                  <motion.button
                    onClick={() => rainSoundInputRef.current?.click()}
                    whileHover={{ scale: 1.02 }}
                    whileTap={{ scale: 0.98 }}
                    className="flex flex-col items-center justify-center gap-3 p-4 bg-white/5 border border-white/10 rounded-xl hover:bg-orange-500/10 hover:border-orange-500/30 transition-all group"
                  >
                    <Volume2 className="text-white/40 group-hover:text-orange-400 transition-colors" size={24} />
                    <span className="text-[10px] uppercase tracking-widest font-bold">Upload Rain Sound</span>
                  </motion.button>
                </div>
                ...
                <div className="mt-6 flex flex-col md:flex-row items-center justify-between gap-4">
                    ...
                    <AnimatePresence mode="wait">
                      {uploadSuccess && (
                        <motion.div
                          ...
                          animate={{ opacity: 1, x: 0 }}
                          exit={{ opacity: 0, x: 20 }}
                          className="flex items-center gap-3 bg-green-500/20 border border-green-500/30 px-4 py-2 rounded-lg"
                        >
                          <div className="w-2 h-2 rounded-full bg-green-500 animate-pulse" />
                          <span className="text-[10px] font-bold uppercase tracking-widest text-green-400">
                            Aset {uploadSuccess === 'bg' ? 'Background' : 'Video'} Berhasil Diunggah!
                          </span>
                          <button 
                            onClick={() => setUploadSuccess(null)}
                            className="ml-2 px-3 py-1 bg-green-500 text-black text-[9px] font-black uppercase rounded hover:bg-green-400 transition-colors"
                          >
                            OKE
                          </button>
                        </motion.div>
                  ...
                  <p className="text-[9px] text-white/30 italic uppercase tracking-tighter">
                    * Aset yang diunggah akan langsung diterapkan pada Layer 2 ini.
                  </p>
                </div>
              </motion.div>
            )}
          ...
          <motion.div 
            ...
            className="absolute inset-0 z-[90] flex flex-col items-center justify-center text-center bg-black overflow-hidden"
          >
            {state.isDevMode && (
              <motion.div 
                initial={{ opacity: 0, y: -20 }}
                animate={{ opacity: 1, y: 0 }}
                className="absolute top-4 right-4 z-[100] group"
              >
                <div className="bg-black/40 backdrop-blur-md border border-white/10 p-2 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity">
                  <motion.button
                    onClick={() => ktpInputRef.current?.click()}
                    whileHover={{ scale: 1.05 }}
                    whileTap={{ scale: 0.95 }}
                    className="flex items-center gap-2 px-4 py-2 bg-white/5 hover:bg-orange-500/20 rounded-lg border border-white/10 transition-all"
                  >
                    <Camera size={14} className="text-white/60" />
                    <span className="text-[9px] font-bold uppercase tracking-widest text-white/60">Update Gambar</span>
                  </motion.button>
                </div>
                <div className="w-8 h-8 flex items-center justify-center bg-white/5 border border-white/10 rounded-full ml-auto group-hover:hidden">
                  <Sliders size={12} className="text-white/20" />
                </div>
              </motion.div>
            )}

            {/* Visual Layer 3 - Only Uploaded Image with Zoom Loop */}
            <motion.div 
            ...
            >
              {state.ktpImage ? (
                ...
                <div className="flex flex-col items-center gap-4">
                  ...
                  <p className="text-[10px] uppercase tracking-[0.3em] text-white/40">Menunggu Upload Gambar di Dev Mode...</p>
                </div>
              )}
            </motion.div>
          </motion.div>
          ...
          <motion.div 
            ...
            className="absolute inset-0 z-[95] flex flex-col items-center justify-center text-center bg-black p-12"
          >
            <motion.p
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 2 }}
              className="text-xl md:text-3xl text-white/90 leading-relaxed font-medium tracking-wide italic max-w-3xl"
            >
              (Suara Batin Yoyo): "Tiga puluh dua tahun. Usia di mana seharusnya aku sudah membangun atap untukmu, Dewi."
            </motion.p>
            ...
            {state.isDevMode && (
              <motion.div 
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                className="absolute bottom-12 z-[100]"
              >
                <motion.button
                  onClick={() => layer4AudioInputRef.current?.click()}
                  whileHover={{ scale: 1.05 }}
                  whileTap={{ scale: 0.95 }}
                  className="flex items-center gap-2 px-6 py-3 bg-white/5 hover:bg-orange-500/20 rounded-full border border-white/10 transition-all"
                >
                  <Music size={16} className="text-orange-500" />
                  <span className="text-[10px] font-bold uppercase tracking-widest text-white/80">Upload Audio Suara Batin</span>
                </motion.button>
              </motion.div>
            )}
          </motion.div>
          ...
          <input 
            type="file" 
            ref={layer4AudioInputRef} 
            onChange={handleLayer4AudioUpload} 
            className="hidden" 
            accept="audio/*"
          />
          <input 
              ...
              <div className="space-y-4">
                <div className="p-4 bg-white/5 rounded-xl border border-white/5">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-[10px] font-mono">
                    ...
                    <div className="text-orange-400">SCENE: {state.status.toUpperCase()}</div>
                    <div className="text-orange-400">VER: 1.1.0-DEV</div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button 
                    onClick={() => fileInputRef.current?.click()}
                    className="py-3 bg-orange-600/20 border border-orange-500/30 rounded-xl text-[10px] font-bold uppercase tracking-widest text-orange-400 hover:bg-orange-500/30 transition-colors flex items-center justify-center gap-2"
                  >
                    <Camera size={14} /> Upload BG
                  </button>
                  <button 
                    onClick={() => videoInputRef.current?.click()}
                    className="py-3 bg-orange-600/20 border border-orange-500/30 rounded-xl text-[10px] font-bold uppercase tracking-widest text-orange-400 hover:bg-orange-500/30 transition-colors flex items-center justify-center gap-2"
                  ...
                  <button 
                    ...
                    className="py-3 bg-orange-600/20 border border-orange-500/30 rounded-xl text-[10px] font-bold uppercase tracking-widest text-orange-400 hover:bg-orange-500/30 transition-colors flex items-center justify-center gap-2"
                  >
                    <Music size={14} /> Menu Music
                  </button>
                  <button 
                    onClick={() => gameSoundtrackInputRef.current?.click()}
                    className="py-3 bg-orange-600/20 border border-orange-500/30 rounded-xl text-[10px] font-bold uppercase tracking-widest text-orange-400 hover:bg-orange-500/30 transition-colors flex items-center justify-center gap-2"
                  >
                    <Music size={14} /> Game Music
                  </button>
                  <button 
                    onClick={() => rainSoundInputRef.current?.click()}
                    className="py-3 bg-orange-600/20 border border-orange-500/30 rounded-xl text-[10px] font-bold uppercase tracking-widest text-orange-400 hover:bg-orange-500/30 transition-colors flex items-center justify-center gap-2"
                  >
                    <Volume2 size={14} /> Rain Sound
                  </button>
                  <button 
                    onClick={() => layer4AudioInputRef.current?.click()}
                    className="py-3 bg-orange-600/20 border border-orange-500/30 rounded-xl text-[10px] font-bold uppercase tracking-widest text-orange-400 hover:bg-orange-500/30 transition-colors flex items-center justify-center gap-2"
                  >
                    <Music size={14} /> Batin Audio
                  </button>
                </div>
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <div className="flex gap-2">
                      ...
                      {state.layer4Audio && (
                        <button 
                          onClick={async () => {
                            setState(prev => ({ ...prev, layer4Audio: undefined }));
                            await removeAsset('sosis_love_layer4_audio');
                          }}
                          className="flex-1 py-3 bg-red-600/20 text-red-400 rounded-xl text-[10px] font-bold uppercase tracking-widest hover:bg-red-600/30 transition-colors"
                        >
                          Clear Batin Audio
